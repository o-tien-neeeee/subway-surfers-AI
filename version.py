...
CHANGELOG = [
    "1.24.0 — DEEP MDP/VISION FIX (nền tảng cho toàn bộ thuật toán phía trên): (1) **ROOT CAUSE — policy mù chân trời**: observation trước là crop 75% DƯỚI màn hình (`ground = image[split:]`) — vật cản spawn ở CHÂN TRỜI (25% trên) bị cắt khỏi CNN, chỉ lọt mắt khi còn <1.5s, nên dù reward đã chỉnh nhiều vòng agent vẫn không thể học đúng. Giờ `perception.ZonePreprocessor` đưa NGUYÊN khung game (full-frame) resize 84×84 làm observation chính thức cho actor/BC/replay (giữ tên trường `ground_gray` cho tương thích, chỉ đổi nội dung); dải chân trời chỉ còn dùng cho bộ báo động frame-diff. (2) **ε đồng hồ với MDP**: actor quyết định theo cadence 2-4 frame/lần (ActionScheduler, giữ action = frame-skip), nhưng epsilon lại được tính theo env_frame_id thô → exploration tụt nhanh gấp 2-4× so với số quyết định policy thực trải nghiệm. Giờ epsilon decay theo SỐ QUYẾT ĐỊNH (`scheduler.stats.decisions`) — đúng time-step của agent; frame-skip cadence của ActionScheduler giữ nguyên (đã đúng). (3) **obstacle_perception.py (mới)**: ObstacleTracker — lưới occupancy 3 làn × 5 mức xa-gần bằng CV thuần numpy (<0.2ms) phát hiện vật cản làn người chơi → tín hiệu danger/clear cho shaping, KHÔNG đụng thuật toán hiện có (DQfD/Rainbow/SIL/IBRL/Dreamer...). Thêm tests/test_obstacle_perception.py (5 tests). Đây là các fix MDP nền tảng mà deep research chứng minh bắt buộc (cả lunai/CS386/subwAI đều dùng full-frame; Atari frame-skip định nghĩa step theo quyết định).",
